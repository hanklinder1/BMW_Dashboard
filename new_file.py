...
def map_excel_to_params(excel_vals: dict, params):
    ...
    Handles both new full names and legacy short names.
    """
    ...
    val = get_first(excel_vals, ["mlops_usd_per_model_year_before", "beta_ops_yr_before"])
    if val is not None:
        params.mlops_usd_per_model_year_before = val
    val = get_first(excel_vals, ["models_deployed_before", "n_models_before"], as_int=True)
    if val is not None:
        params.models_deployed_before = val
    ...
    if val is not None:
        params.labels_per_year_after = val
    ...
    if val is not None:
        params.models_deployed_after = val
